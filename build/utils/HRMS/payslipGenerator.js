...
export const generatePayslipFile = async (request, reply) => {
    ...
    for (const e of data) {
        ...
        console.log(e);
        let payslipData = await fileGeneration(e, request.protocol, request.headers.host);
        // let fileurl =
        //   request.protocol +
        //   "://" +
...
const fileGeneration = async (data, protocol, host) => {
    ...
    const content = await fs.promises.readFile(path.resolve("CD_paySlip.docx"), "binary");
    ...
    const docxFilePath = path.resolve(__dirname, "..", "..", "..", "uploads", `${data.name}_PaySlip_${data.paySlipMonth}_${data.paySlipYear}.docx`);
    const pdfFilePath = path.resolve(__dirname, "..", "..", "..", "uploads", `${data.name}_PaySlip_${data.paySlipMonth}_${data.paySlipYear}.pdf`);
