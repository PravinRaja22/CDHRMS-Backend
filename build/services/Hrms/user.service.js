...
(function (userService) {
    ...
    userService.getAuthorizedUserdata = async (username) => {
        try {
      ...
      WHERE LOWER(users.username) = LOWER($1)
            ...
            let data = await query(joinQuery, params);
            ...
            if (data.rows.length > 0) {
                ...
                let upsertResult = userService.upsertUser({ userName: username });
    ...
    userService.upsertUser = async (request) => {
        try {
            ...
            if (id) {
                ...
                querydata = `UPDATE users SET ${fieldNames
                ...
                querydata = `INSERT INTO users (${fieldNames.join(", ")}) VALUES (${fieldNames
                    .map((_, index) => `$${index + 1}`)
                    .join(", ")})`;
                params = fieldValues;
            }
    ...
    async function deleteUser(id) {
        try {
            ...
            // Check user exists
            const findUser = await query("SELECT * FROM users WHERE id = $1", [
                userId,
            ]);
    ...
    userService.getUsersBankdetails = async (userId) => {
    ...
    userService.getUsersPFdetails = async (userId) => {
    ...
    userService.getUsersMedicalInsurence = async (userId) => {
    ...
    userService.getUsersByQueries = async (reqQuery) => {
        ...
        try {
            ...
            const conditions = keys
                .map((key, index) => `LOWER(${key}) LIKE LOWER($${index + 1})`)
                .join(" AND ");
