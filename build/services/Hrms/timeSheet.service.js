...
export var timeSheetServices;
(function (timeSheetServices) {
    async function gettimeSheetServices(userId) {
        try {
            console.log("gettimeSheetServices call");
            const result = await query(`SELECT * FROM timesheets WHERE userId = $1 ORDER BY date ASC`, [userId.userId]);
    ...
    timeSheetServices.gettimeSheetServices = gettimeSheetServices;
    async function getTimeSheetbydateanduser(userId, date) {
        try {
            console.log("gettimeSheetServices call");
            console.log(userId, date);
            const result = await query(`SELECT * FROM timesheets where userId = ${userId} And date = ${date} `, []);
            console.log(result, "query results");
            return result.rows;
        }
        catch (error) {
    ...
    timeSheetServices.getTimeSheetbydateanduser = getTimeSheetbydateanduser;
    async function getTimeSheetForMonthandYear(userId, Month, Year) {
        try {
            ...
            const result = await query(`SELECT * FROM timesheets where date >= ${data.startTime} And date <=${data.endTime} And userId = ${Number(userId)}  ORDER BY date ASC`, []);
    ...
    timeSheetServices.getTimeSheetForMonthandYear = getTimeSheetForMonthandYear;
    async function excelGenearator(request, reply) {
        try {
    ...
    timeSheetServices.excelGenearator = excelGenearator;
    async function upserttimeSheetServices(data) {
        try {
            console.log(data);
    ...
    timeSheetServices.upserttimeSheetServices = upserttimeSheetServices;
