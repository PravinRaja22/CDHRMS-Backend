...
export var PayslipServices;
(function (PayslipServices) {
    async function generatePayslip(request) {
        ...
        try {
            ...
            if (getAttendance.rowCount > 0) {
                ...
                let generatePayslip = await PayslipServices.calculatePayslip(getAttendance.rows, totalNumberOfDays, request);
    ...
    PayslipServices.generatePayslip = generatePayslip;
    async function generateBulkPayslip(request) {
            ...
            if (result.rowCount > 0) {
                ...
                for (const item of result.rows) {
                    ...
                    if (getAttendance.rowCount > 0) {
                        ...
                        let payslipAmount = await PayslipServices.calculatePayslip(getAttendance.rows, totalNumberOfDays, request);
    ...
    PayslipServices.generateBulkPayslip = generateBulkPayslip;
    PayslipServices.calculatePayslip = async (attendanceRecords, totalNumberOfDaysMonth, request) => {
        ...
        try {
            ...
            console.log(new Date(Number(userRecord?.joiningdate))?.toJSON()?.slice(0, 10), "USER DATA date");
            let obj = {
                userId: `${userRecord?.id}`,
                name: `${userRecord?.firstname} ${userRecord?.lastname === null ? "" : userRecord?.lastname}`,
                paySlipMonth: month,
                paySlipYear: year,
                employeeNo: userRecord?.employeeid,
                joiningDate: new Date(Number(userRecord?.joiningdate))?.toJSON()?.slice(0, 10) ||
                    null,
    ...
    async function insertpaySlip(data) {
        ...
        try {
            ...
            if (findMatchingdata.rows.length > 0) {
                ...
                try {
                    ...
                    console.log(upsertPayslipResult, "********** upsertPayslipResult result");
                }
                catch (error) {
                    console.log("catch");
                ...
                let params = [];
                // If id is not provided, insert a new scheduled interview
                querydata = `INSERT INTO payslips (${fieldNames.join(", ")}) VALUES (${fieldValues
                    .map((_, index) => `$${index + 1}`)
                    .join(", ")}) RETURNING *`;
                params = fieldValues;
    ...
    PayslipServices.insertpaySlip = insertpaySlip;
    async function getPayslip(request) {
        console.log(request);
    ...
    PayslipServices.getPayslip = getPayslip;
    async function getPayslipByUserMonth(request) {
    ...
    PayslipServices.getPayslipByUserMonth = getPayslipByUserMonth;
    async function getAllPaySlipData(request) {
    ...
    PayslipServices.getAllPaySlipData = getAllPaySlipData;
    async function generateBulkPayslipData(request) {
        console.log("generateBulkPayslip control");
        console.log("generateBulkPayslip", request);
        const { month, year, utcSec } = request.params;
        let startDate;
    ...
    PayslipServices.generateBulkPayslipData = generateBulkPayslipData;
