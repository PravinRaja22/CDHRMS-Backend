...
(function (attendanceService) {
    ...
    attendanceService.getAttendanceData = getAttendanceData;
    ...
    attendanceService.upsertAttendance = upsertAttendance;
    async function getAttendanceByUserIdDate(request) {
        console.log("getAttendanceByUserIdDate");
        let params = request.params || request;
        console.log(params, "params");
        const existingRecord = await query("SELECT * FROM attendances WHERE userId = $1 AND date = $2", [params.userId, params.attendanceDate]);
    ...
    attendanceService.getAttendanceByUserIdDate = getAttendanceByUserIdDate;
    async function getAttendanceByUserIdMonth(params) {
        try {
            ...
            console.log(result);
            const queryData = await query(`SELECT * FROM attendances WHERE userId = $1 AND date >=${result.startTime} AND date <= ${result.endTime}`, [userId]);
            console.log(queryData, "query result");
            return queryData.rows;
        }
        catch (error) { }
    }
    attendanceService.getAttendanceByUserIdMonth = getAttendanceByUserIdMonth;
    async function updateAttendance(params, body) {
            ...
            try {
                ...
                if (result.command === "UPDATE" && result.rowCount > 0) {
                    ...
                    if (attendance?.signout.data[0]?.timeStamp && attendance?.signout.data.length === attendance?.signin.data.length) {
                        ...
                        let updateAttendanceStatus = attendanceService.updateAttendanceStatus(params);
                ...
                let message = result.command === "UPDATE" && result.rowCount > 0
    ...
    async function getsingleAttendance(recId) {
        console.log("getsingleAttendance", recId);
        try {
            const result = await query(`SELECT * FROM attendances WHERE id = ${recId}`, []);
    ...
    attendanceService.getsingleAttendance = getsingleAttendance;
    //attendance bulk insert
    async function upsertBulkAttendance(values) {
        try {
            ...
            while (currentDate <= endDate) {
                ...
                if (!existRecord) {
                    const record = {
                            ...
                            data: [
                                {
                                    ...
                                    timeStamp: currentDate.getDay() === 0 || currentDate.getDay() === 6
                                        ? null
                                        : null,
                                    lat: 0,
                                    lng: 0,
                        ...
                        status: currentDate.getDay() === 0 || currentDate.getDay() === 6
                            ? "weekoff"
                            : null,
                        shift: {
                            shiftType: "GS",
    ...
    attendanceService.upsertBulkAttendance = upsertBulkAttendance;
    async function upsertAttendanceRecord(record) {
        ...
        let querydata = `INSERT INTO attendances (${fieldNames.join(", ")}) VALUES (${fieldValues
            .map((_, index) => `$${index + 1}`)
            .join(", ")}) RETURNING *`;
    ...
    attendanceService.upsertAttendanceRecord = upsertAttendanceRecord;
    ...
    //     }
    // }
    async function updateAttendanceStatus(params) {
        console.log(params, "updateAttendanceStatus params");
        let attendanceDate = params.attendanceDate;
        ...
        try {
            ...
            if (result.rowCount > 0) {
                ...
                const updatePromises = attendanceRecords.map(async (i) => {
                    console.log(i, "______________");
                    let updatedRecord1 = await calculateAttendance(i);
                    console.log(updatedRecord1, "calculateAttendance result");
                    if (updatedRecord1) {
                        let { uuid, ...updatedRecord } = updatedRecord1;
                ...
                returnValue = {
                    ...
                    message: updatesqlCount && failuresqlCount
    ...
    attendanceService.updateAttendanceStatus = updateAttendanceStatus;
    async function sqlUpdate(updatedRecord) {
    ...
    async function calculateAttendance(item) {
        ...
        let dateA = new Date(Number(signIns[0].timeStamp));
        let dateB = new Date(Number(signOuts[0].timeStamp));
    ...
    async function generateAttendanceData(values) {
        ...
        try {
            ...
            for (const user of allUsers.rows) {
                try {
                    ...
                    //    existingRecord
                    const existingRecord = await query("SELECT * FROM attendances WHERE userId = $1 AND date = $2", [user.id, newDateUTC]);
    ...
    async function getAttendaceForMonthandYear(userId, Month, Year) {
        try {
            console.log(userId, Month, Year);
            console.log("attendanceService call");
            let data = await getStartandEndTIme(Month, Year);
            console.log(data.startTime);
            const result = await query(`SELECT * FROM attendances where date >= ${data.startTime} And date <=${data.endTime} And userId = ${Number(userId)}`, []);
    ...
    attendanceService.getAttendaceForMonthandYear = getAttendaceForMonthandYear;
    ...
    async function upsertAttendanceTime(data, userId) {
        try {
            console.log(data);
            console.log(userId);
            console.log("upsert TIme in Attendance date");
            console.log(data.signInDate);
            const result = await query(`SELECT * FROM attendances where date = ${data.signInDate} And userId = ${Number(userId)}`, []);
            console.log(result.rows);
    ...
    attendanceService.upsertAttendanceTime = upsertAttendanceTime;
    const insertUpdateAttendance = async (updatedRecord) => {
        ...
        try {
            ...
            if (updatedRecord?.id) {
                ...
                querydata = `INSERT INTO attendances (${fieldNames.join(", ")}) VALUES (${fieldValues
                    .map((_, index) => `$${index + 1}`)
                    .join(", ")}) RETURNING *`;
                params = [...fieldValues];
            }
    ...
    async function upsertAttendanceforLeaves(values) {
    ...
    attendanceService.upsertAttendanceforLeaves = upsertAttendanceforLeaves;
    async function caculateAttendaceForMonth(month, year, userId) {
        try {
            ...
            if (getAttendance.rowCount > 0) {
                let joinUsersResult = await userService.getSingleUser(userId);
    ...
    attendanceService.caculateAttendaceForMonth = caculateAttendaceForMonth;
