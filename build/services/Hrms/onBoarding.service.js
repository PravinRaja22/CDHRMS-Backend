...
(function (onboardingService) {
    ...
    onboardingService.getAllOnboardingData = getAllOnboardingData;
    async function getSingleOnboardingData(id) {
        try {
            console.log("onboardingService - getSingleOnboardingData call");
            const result = await query("SELECT * FROM onboarding WHERE id = $1", [id]);
            console.log(result.rows, "single onboarding record");
            return result.rows[0];
        }
        catch (error) {
            console.error(`Error in getSingleOnboardingData for id ${id}:`, error.message);
            return { error: error.message };
        }
    }
    onboardingService.getSingleOnboardingData = getSingleOnboardingData;
    async function upsertOnboardingData(request) {
        try {
            ...
            if (id) {
                ...
                querydata = `INSERT INTO onboarding (${fieldNames.join(", ")}) VALUES (${fieldNames
                    .map((_, index) => `$${index + 1}`)
                    .join(", ")})`;
                params = fieldValues;
            }
    ...
    onboardingService.upsertOnboardingData = upsertOnboardingData;
    async function deleteOnboardingData(id) {
        try {
            console.log("onboardingService - deleteOnboardingData call");
            const result = await query("DELETE FROM onboarding WHERE id = $1 RETURNING *", [id]);
            ...
            console.error(`Error in deleteOnboardingData for id ${id}:`, error.message);
            return { error: error.message };
        }
    }
    onboardingService.deleteOnboardingData = deleteOnboardingData;
