...
export var otherChaptersService;
(function (otherChaptersService) {
    async function getAllOtherChaptersData() {
        try {
    ...
    otherChaptersService.getAllOtherChaptersData = getAllOtherChaptersData;
    async function getOtherChaptersDataById(id) {
        try {
    ...
    otherChaptersService.getOtherChaptersDataById = getOtherChaptersDataById;
    async function getOtherChaptersDataByUserId(userId) {
        try {
            console.log(`Fetching Other Chapters data for userId: ${userId}`);
            const result = await query("SELECT * FROM otherChapters WHERE userId = $1", [userId]);
            console.log("Fetched Other Chapters Data:", result.rows);
            return result.rows;
        }
        catch (error) {
    ...
    otherChaptersService.getOtherChaptersDataByUserId = getOtherChaptersDataByUserId;
    async function upsertOtherChaptersData(request) {
        try {
            ...
            if (id) {
                ...
                querydata = `INSERT INTO otherChapters (${fieldNames.join(", ")}) VALUES (${fieldNames
                    .map((_, index) => `$${index + 1}`)
                    .join(", ")}) RETURNING *`;
                params = fieldValues;
            }
    ...
    otherChaptersService.upsertOtherChaptersData = upsertOtherChaptersData;
    async function deleteOtherChaptersData(id) {
        try {
            console.log(`Deleting Other Chapters data for userId: ${id}`);
            const result = await query("DELETE FROM otherChapters WHERE id = $1 RETURNING *", [id]);
    ...
    otherChaptersService.deleteOtherChaptersData = deleteOtherChaptersData;
})(otherChaptersService || (otherChaptersService = {}));
//# sourceMappingURL=otherChapters.service.js.map