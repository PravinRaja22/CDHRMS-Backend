...
(function (approvalService) {
    ...
    approvalService.getAllApprovals = getAllApprovals;
    async function insertApprovals(values, objectName) {
        ...
        try {
            ...
            let querydata = `INSERT INTO approvals (${fieldNames.join(", ")}) VALUES (${fieldNames
                .map((_, index) => `$${index + 1}`)
                .join(", ")}) RETURNING *`;
    ...
    approvalService.insertApprovals = insertApprovals;
    async function getApprovalsById(recId) {
    ...
    approvalService.getApprovalsById = getApprovalsById;
    async function getApprovalbyApprover(approverId) {
        try {
    ...
    approvalService.getApprovalbyApprover = getApprovalbyApprover;
    async function getApprovalsByApproverLeaveQuery(userId, reqQuery) {
      ...
      WHERE ${conditions} AND approvals.approverid = $${keys.length + 1}`;
            ...
            const result = await query(innerQuery, newParams);
    ...
    approvalService.getApprovalsByApproverLeaveQuery = getApprovalsByApproverLeaveQuery;
    async function getApprovalsByApproverAttendanceQuery(userId, reqQuery) {
        console.log("getApprovalsByApproverLeaveQuery");
        let keys = Object.keys(reqQuery);
        let values = Object.values(reqQuery);
        console.log(keys, values, "getApprovalsByApproverLeaveQuery");
    ...
    approvalService.getApprovalsByApproverAttendanceQuery = getApprovalsByApproverAttendanceQuery;
    async function updateApprovals(requestBody, requestParams) {
    ...
    approvalService.updateApprovals = updateApprovals;
        ...
        if (approvalRec.type.toLowerCase().includes("attendance")) {
            try {
                ...
                if (updateRegularize?.status === 200) {
                    ...
                    //update attendance
                    updateAttendanceResult = await updateAttendance(newObj, "attendanceRegularize");
    ...
    const updateLeaveBalance = async (leaveRec) => {
        ...
        let leaveBalance = await leaveBalanceService.getLeaveBalanceByUsers(leaveRec.userid);
        console.log(leaveBalance, "leaveBalance");
        let year = new Date(Number(leaveRec.fromdate)).getFullYear();
        console.log(new Date(Number(leaveRec.fromdate)).getFullYear(), "leaverec fromDate year");
        ...
        let upsertLeaveBalance = await leaveBalanceService.upsertLeaveBalanceByUsers(leaveRec.userid, leaveBalance[0]);
...
const calculateAttendance = async (signinDates, updatedAttendanceRecord) => {
    ...
    console.log("*******");
    let dateA = new Date(Number(signIns));
    let dateB = new Date(Number(signOuts));
