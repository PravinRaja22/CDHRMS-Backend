...
export var payslipService;
(function (payslipService) {
    payslipService.generateFile = async (data) => {
...
const fileGeneration = async (data) => {
    const content = await fs.readFile(path.resolve("src/services/Hrms/CD_paySlip.docx"), "binary");
    ...
    const pdfFilePath = path.resolve(`${data.name}_PaySlip_${data.paySlipMonth}_${data.paySlipYear}.pdf`);
    const docxFilePath = path.resolve(`${data.name}_PaySlip_${data.paySlipMonth}_${data.paySlipYear}.docx`);
