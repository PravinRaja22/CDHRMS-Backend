...
export module medicalInsuranceService{
    ...
    export const upsertMedicalInsurances = async(reqBody:object)=>{
   
        console.log(reqBody, "upsertMedicalInsurances ");
        const { id, ...otherFields }:any = reqBody;
        ...
        if(id){
          ...
          .join(", ")} WHERE id = $${fieldNames.length + 1} RETURNING *`;
            ...
            if(varifyUsers.status===200){
                querySQL = `INSERT INTO medicalInsurances (${fieldNames.join(
                    ", "
                  )}) VALUES (${fieldNames
                    .map((_, index) => `$${index + 1}`)
                    .join(", ")}) RETURNING *`;
    ...
    export const getMedicalInsurancesById =async (recId)=>{
    ...
    export const deleteMedicalInsurancesById =async (recId)=>{
