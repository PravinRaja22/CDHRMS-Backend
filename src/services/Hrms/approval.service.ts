...
export module approvalService {
  ...
  export async function updateApprovals(requestBody: any, requestParams: any) {
    ...
    try {
      query = `UPDATE approvals SET ${fieldNames
      ...
      if (result.command === "UPDATE" && result.rowCount > 0) {
        ...
        return {
          message: `${requestBody.type.label} ${requestBody.status.label} Successfully`,
        };
      } else {
  ...
  async function updateParentRecord(values, approvalRecId) {
    ...
    if (values.type.value === "attendanceRegularizations") {
    ...
    } else if (values.type.value === "leaves") {
      ...
      try {
        ...
        let updateAttendance = await updateAttendanceForLeave(newLeave);
        console.log(
          updateAttendance,
  ...
  const updateAttendance = async (attendanceRegularizeRecord) => {
    ...
    let getAttendanceRecord = await attendanceService.getAttendanceByUserIdDate(
      params
    );
    ...
    try {
      let result: any = attendanceService.updateAttendance(
        params,
        newAttendance
      );
  ...
  async function updateAttendanceForLeave(leaveRecord) {
    console.log(leaveRecord, "leaveRecord from updateAttendanceForLeave");
    let { userDetails, date, ...other } = leaveRecord;
    let params = { userId: userDetails.id, attendanceDate: date };
    let getAttendanceRecord = await attendanceService.getAttendanceByUserIdDate(
      params
    );
    console.log(
      getAttendanceRecord,
      "getAttendanceRecord from updateAttendanceForLeave"
    );
  }

  async function updateLeaveBalanceForLeave(leavebalanceRecord) {
    console.log(
      leavebalanceRecord,
      "leavebalanceRecord from updateLeaveBalanceForLeave"
    );
  }
}
